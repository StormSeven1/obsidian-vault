...
[![image](https://cdn3.linux.do/optimized/4X/6/b/d/6bdd0b283f2cf5fb3a2d72e8705b016a540dd862_2_689x203.jpeg)

image 1196×352 71.6 KB

](https://cdn3.linux.do/original/4X/6/b/d/6bdd0b283f2cf5fb3a2d72e8705b016a540dd862.jpeg "image")
...
> 
> `{  "env": {    "HTTPS_PROXY": "http://127.0.0.1:7890",    "HTTP_PROXY": "http://127.0.0.1:7890",    "CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC": "1",    "CLAUDE_CODE_ATTRIBUTION_HEADER": "0",    "DISABLE_INSTALLATION_CHECKS": "1",    "ENABLE_TOOL_SEARCH": "1"  },  "cleanupPeriodDays": 720,  "model": "opus[1m]" }`
...
- `ENABLE_TOOL_SEARCH`： 启用 ToolSearch 这一 Tool，该 Tool 的能力是将 Tool 们全部延后加载 在需要时会自动先调用该工具进行 Tool 的搜索加载 （因为默认是直接加载到上下文窗口的）
    ...
    - 可加可不加 最好不写 因为在 1 M 上下文窗口实际上不是特别有必要了
...
以下以 anyr 0 uter 为例进行讲解  
创建 `~/.claude/settings.anyrouter.json`

> ~/.claude/settings.anyrouter.json
> 
> `{  "env": {    "ANTHROPIC_BASE_URL": "https://anyrouter.top",    "ANTHROPIC_AUTH_TOKEN": "you-token-to-here"     },  "model": "opus[1m]" }`
...
所以根本无须考虑 切换使用 Sonnet 模型 则直接锁定在推荐的 1 M 模型上  
...
> 
> `{  "env": {    "ANTHROPIC_BASE_URL": "https://dashscope.aliyuncs.com/apps/anthropic",    "ANTHROPIC_API_KEY": "you-token-to-here",    "ANTHROPIC_DEFAULT_HAIKU_MODEL": "qwen3.5-flash",    "ANTHROPIC_DEFAULT_SONNET_MODEL": "qwen3.5-flash",    "ANTHROPIC_DEFAULT_OPUS_MODEL": "qwen3.5-plus"  } }`
...
A：CC 默认会追加 `/v1/messages` 拼接 进行 LLM 请求，所以 BASE_URL 就是指 抠掉这块之前的那部分 动动你的小脑瓜 不要想当然无脑 往上自己追加 /v 1 后缀
...
Q: 响应码 4 xx （no body）?  
A: 切模型到 Opus 1 M （盲猜 Opus 没米了）

- 就算提问也不要光说自己得到了什么响应码错误，没有响应码之神 可以透过响应码贯穿得知响应的 message，这很重要
- 那么如何得知 message 呢 POST 打一下也就知道了 如果有的话 比如现在没有米的情况是  
    
    [![image](https://cdn3.linux.do/optimized/4X/f/f/a/ffa5634bc91330c55851376c2b37709ae1cffec6_2_690x285.png)
    
    image 1408×582 122 KB
    
    ](https://cdn3.linux.do/original/4X/f/f/a/ffa5634bc91330c55851376c2b37709ae1cffec6.png "image")
...
![[Anyrouter 使用指北-1778062627702.webp]]